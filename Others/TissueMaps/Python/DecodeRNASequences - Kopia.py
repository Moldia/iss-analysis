...
class DecodeRNASequences(object):
    ...
    def decode(self, num_hybs, quality_Th_D, DO_intensity_Th_D, CPcsv_name, Output_path, output_prefix, taglist_file):
        ...
        for t in range(0,len(tiles)):
            ...
            tile_num_cells = int(max(tile_seq_data[:,13]))

            for i in range(0, tile_num_blobs):                
                pp = tile_seq_data[tile_seq_data[:,1] == i+1, 7:11] # collects data from all hybsteps of given blob tile
                if len(pp) == num_hybs:
                    hyb0row = tile_seq_data[tile_seq_data[:,1] == i+1, :]
                    hyb0row = hyb0row[hyb0row[:,2] == "hyb0", :]
                    x = hyb0row[3] + hyb0row[11]
                    y = hyb0row[4] + hyb0row[12]
                    ...
                    if tile_seq_data[i,13]>0:
                        seq_cell_ID[i+start_blob_ID] = start_cell_ID+tile_seq_data[i,13]
        ...
        binEdges = np.append(hh, hh[-1] + 1) # add rightmost edge
        ...
        for n in range(0,nExpected):
            expected_list.append(taglist[n][1])
            exp_tags.append(taglist[n][0])
            [token, remain] = exp_tags[n].split(' ', 1)
            exp_tags[n] = remain
        # --------------------------------------------
        ...
        for kk in range(0, len(b)):
             if b[kk] in expected_list:
                 posit = expected_list.index(b[kk])
        ...
        for kkk in range(0, len(all)):
            if all[kkk] in expected_list:
                posit = expected_list.index(all[kkk])
        ...
        for kkkk in range(0, len(allbt)):
            if allbt[kkkk] in expected_list:
                posit = expected_list.index(allbt[kkkk])
        ...
        # print a title, followed by a blank line
        fid.write('digits letters name X_pos Y_pos parent_cell seq_quality\n\n')
        # print values in column order
        for row in range(0, len(all)):
            fid.write('%d %s %s %d %d %d %.7f\n' %(all[row], all_letters[row], name_tag_all[row], np.floor(x_all[row]), np.floor(y_all[row]), par_cell_all[row], seq_quality_min_all[row]))
        fid.close()
        ...
        for row in range(0, len(allbt)):
            fid.write('%d %s %s %d %d %d %.7f\n' %(allbt[row], all_lettersbt[row], name_tag_allbt[row], np.floor(x_allbt[row]), np.floor(y_allbt[row]), par_cell_allbt[row], seq_quality_min_allbt[row]))
        fid.close()
