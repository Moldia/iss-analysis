image_directory = 'F:\HCA_07_DG';
image_files = {
    'HCA_07_cycle1_b(2).czi'
    'HCA_07_cycle2_b(2).czi'
    'HCA_07_cycle3(2).czi'
    '190112_HCA_07_cyc4(2).czi'};
...
% window is closed)
inspect_images(fullfile(output_directory, 'ReslicedTiles'), image_files, nTiles, 2:6)

% preliminary base calling
Results = test_analysis(fullfile(output_directory, 'ReslicedTiles'), image_files, output_directory, nTiles, [6 2 3 4], 'taglist.csv');
...
for s = 1:numel(files)
    for c = 1:size(stitched_files,1)
        ...
        tileimage(I, tile_size, fullfile(outdir, output_prefix));
    end
end
...
randomTiles = randperm(nTiles, 10);
Stitched = ls(indir);
Stitched = cellstr(Stitched(3:end,:));
