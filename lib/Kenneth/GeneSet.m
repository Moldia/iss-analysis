...
%                 end
    ...
    %                 g.CellInfo = setfield(g.CellInfo,ColAttrName{1}{1}, ColAttrVals{1});
        ...
        function h = ScaleCell(g, p, q);
            ...
            h.GeneExp = bsxfun(@rdivide, g.GeneExp, Norm.^p);
        ...
        function h = horzcat(h, g2, varargin)
            ...
            if nargin>2 % recursion if more than 2 inputs
                h = horzcat(horzcat(h, g2), varargin{:});
                return;
            end
            ...
            if ~isequal(h.GeneName, g2.GeneName)
                error(sprintf('Gene Names mismatch'));
            end
            
            h.GeneExp = [h.GeneExp, g2.GeneExp];
            h.nCells = h.nCells + g2.nCells;
            h.Class = vertcat(h.Class(:), g2.Class(:));
            h.CellName = vertcat(h.CellName(:), g2.CellName(:));
            h.nGenes = g2.nGenes;
            h.GeneInfo= g2.GeneInfo;

            if ~isempty(h.CellInfo) & ~isempty(g2.CellInfo)
                % merge cell info (this is the hard part)
                CellInfoFields = intersect(fieldnames(h.CellInfo), fieldnames(g2.CellInfo));
                NewCellInfo = struct;
                for i=1:length(CellInfoFields(:));
                     f = CellInfoFields{i};
    %                 if isfield(g2.CellInfo, f)
                    c1 = getfield(h.CellInfo,f);
                    if length(c1)~=h.nCells
                        error(sprintf('CellInfo.%s has %d entries instead of %d', ...
                            f, length(c1), h.nCells));
                    end
...
%                 ColorMap(TooYellow,:) = ColorMap(TooYellow,:)*diag([.5 .5 .7]);
            ...
            if length(IDs)== 2 & iscell(IDs)
                ...
                nIDs1 = length(IDs1);
                nIDs2 = length(IDs2);
                [h, ax, bigax] = gplotmatrix(g.GeneExp(IDs1,:)' + rand(g.nCells, nIDs1)*Jitter, ...
                    g.GeneExp(IDs2,:)' + rand(g.nCells, nIDs2)*Jitter, Group, ColorMap, Sym, varargin{:});
                set(ax, 'box', 'off');
                for i=1:nIDs1
                    xlabel(ax(nIDs2,i), g.GeneName{IDs1(i)}, 'Interpreter', 'none'); 
                end
                for i=1:nIDs2
                    ylabel(ax(i,1), g.GeneName{IDs2(i)}, 'Interpreter', 'none'); 
                end
            elseif size(IDs,2)==1 | size(IDs,1)==1
                ...
                set(ax, 'box', 'off');
                for i=1:nIDs
                    xlabel(ax(nIDs,i), g.GeneName{IDs(i)}, 'Interpreter', 'none'); 
                    ylabel(ax(i,1), g.GeneName{IDs(i)}, 'Interpreter', 'none'); 
                end
            else
...
%             end
        ...
        function h = GeneScatter(g, c1, c2, GenesToShow, Jitter, Highlight)
            ...
            if isstr(oc1); xlabel(oc1); else; xlabel('user'); end
            if isstr(oc2); ylabel(oc2); else; ylabel('user'); end
...
%                         h = plot(g.tSNE(1,pts), g.tSNE(2,pts), Sym(i), 'Color', Clr(i,:));
        ...
        function tSNEPlot(g, Gene, SizeMult, SizeOffset, LogAdd, Sym, Clr, Size0)
            ...
            if nargin>=2 && (ischar(Gene) || isscalar(Gene))
