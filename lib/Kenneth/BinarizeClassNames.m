...
% Tree is a tree structure containing the class names (needs @tree toolbox by Jean-Yves Tinevez) 
...
%             ParentNode = find(Tree.strcmp(ParentRoot));
        end
        ...
        % do it in a way that preserves original order
        [c, ia, ic] = unique(ClassToken(My));
        UniqueTokens = ClassToken(My(sort(ia)));
...
%             Tree = Tree.addnode(ParentNode, [ParentRoot '.' MyTokens{1}]);
%             Tree = Tree.addnode(ParentNode, [ParentRoot '.' MyTokens{2}]);
